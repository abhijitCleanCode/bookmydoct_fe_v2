	...
	const handleSlotChange = (name, value, id) => {
		setSchedule(
			schedule.map((schedule) => {
				if (schedule.id == id) {
					if (name === 'startTime' && Number(value.slice(0, 2)) > Number(schedule.endTime.slice(0, 2))) {
						toast.error('Start time cannot be greater than end time');
						return schedule;
					}
					if (name === 'endTime' && Number(schedule.startTime.slice(0, 2)) > Number(value.slice(0, 2))) {
						toast.error('Start time cannot be greater than end time');
						return schedule;
					}
					return {
		...
		<Sheet open={isAddDrawerOpen} onOpenChange={setIsAddDrawerOpen}>
			<SheetContent className='sm:min-w-[600px] overflow-scroll' >
				<SheetHeader className='mb-6'>
					<SheetTitle className='text-2xl font-bold text-primary'>Add New Doctor</SheetTitle>
				</SheetHeader>
								...
								<CardContent className='space-y-4 pt-6'>
									...
									<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
										<div className='space-y-2'>
											...
											<Select
												name='specialization'
												onValueChange={(value) => handleInputChange(value, 'specialization', value)}
