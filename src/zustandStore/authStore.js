    ...
    (set, get) => ({
        ...
        try {
          ...
          if (role == "clinic") {
            toast((t) => (
              <span className="flex">
                <p>
                  Registration successful! Please wait for the super admin to
                  verify details
                </p>
      ...
      login: async (credentials, role) => {
        ...
        try {
          ...
          if (
            role === "clinic" &&
            error.toString().includes("No refresh token")
          ) {
            toast.error(
              "Clinic details are not verified yet pls contact admin"
            );
          } else {
      ...
      resetPassword: async (email, otp, password, role) => {
        try {
          set({ isLoading: true });
          const res = await api.post(`/auth/${role}/reset-password`, {
            email,
            otp,
      ...
      changePassword: async (oldPassword, newPassword, role) => {
        try {
          set({ isLoading: true });
          const res = await api.post(`/auth/${role}/change-password`, {
            oldPassword,
            newPassword,
          });
