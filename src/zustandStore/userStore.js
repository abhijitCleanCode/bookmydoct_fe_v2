...
export const userStore = create((set) => ({
  ...
  getSlotDetails: async (doctorId, clinicId, scheduledId, appointmentDate) => {
    try {
      const res = await api.get(
        `/user/doctor/slots?doctorId=${doctorId}&clinicId=${clinicId}&scheduledId=${scheduledId}&appointmentDate=${appointmentDate}`
      );
      return res.data;
    } catch (error) {
      console.log(error);
  ...
  getClinicDetails: async (clinicId) => {
    try {
      ...
      toast.error(
        error.response?.data?.message || "Error fetching clinic details"
      );
    }
  },
  ...
  getAllAppointments: async (pageNumber) => {
    try {
      set({ isLoading: true });
      const res = await api.get(
        `/user/lab/appointments?page=${pageNumber}&limit=10`
      );
