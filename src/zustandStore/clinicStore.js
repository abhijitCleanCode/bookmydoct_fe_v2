...
const useClinicStore = create(persist((set) => ({
    ...
    deleteDoctor: async (id) => {
        try {
            ...
            set((state) => ({ doctors: state.doctors.filter((doctor) => doctor._id !== id) }));
    ...
    updateClinicDetails: async (clinicDetails) => {
        try {
            ...
            console.log(err);
            toast.error(err.response?.data.message || 'Error updating clinic details')
            set({ isLoading: false });
        }
    },
    ...
    getAllAppointments: async (pageNumber) => {
        try {
            set({ isLoading: true });
            const res = await api.get(`/clinic/labs/appointments?page=${pageNumber}&limit=10`);
            set({ isLoading: false });
            return res.data;
        } catch (err) {
            console.log(err);
